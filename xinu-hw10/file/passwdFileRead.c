 ...
 */
	int size_of_table = MAXUSERS*sizeof(struct userent);
	...
	fileSeek(fd, 0);
	char reading_buffer[size_of_table];
	bzero(reading_buffer, size_of_table);
	int i;
	for (i = 0; i < size_of_table; i++) {
		char c = fileGetChar(fd);
		if (c == SYSERR) {
			printf("passwd file was shorter than we expected\n");
			return SYSERR;
		}
		reading_buffer[i] = c;	
	}
	fileClose(fd);
	memcpy(usertab, reading_buffer, size_of_table);

	if (usertab[SUPERUID].state != USERUSED) {
		printf("Passwd file contents corrupted\n");
		return SYSERR;
	}
	if (usertab[SUPERUID].salt != SALT) {
		printf("Passwd file contents corrupted\n");
		return SYSERR;
	}
